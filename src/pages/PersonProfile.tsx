...
export default function PersonProfile() {
 ...
 const { recordDailyAction } = useGamification();
 ...
 const [isEnrichingAI, setIsEnrichingAI] = React.useState(false);

 const handleBonusAIEnrichment = async () => {
 const targetId = resolvedId || id;
 if (!targetId || !firebaseUser) return;

 const currentCredits = user?.bonusEnrichmentCredits || 0;
 if (currentCredits <= 0) {
 if (confirm("You don't have any Bonus AI Enrichment credits. Would you like to visit the Aura Shop to purchase one for 40 Aura?")) {
 navigate('/shop');
 }
 return;
 }
 ...
 const enrichedText = await generateEnrichedAINotes({
 name: displayPerson?.name || 'Friend',
 interests: displayPerson?.interests || '',
 notes: displayPerson?.notes || '',
 category: displayPerson?.category || 'friend'
 });

 const personRef = doc(db, 'people', targetId);
 await updateDoc(personRef, {
 ai_notes: enrichedText,
 updated_at: serverTimestamp()
 });

 const userRef = doc(db, 'users', firebaseUser.uid);
 await updateDoc(userRef, {
 bonusEnrichmentCredits: increment(-1)
 });

 setPerson((prev: any) => prev ? { ...prev, ai_notes: enrichedText } : prev);
 await refreshUser();
 alert(`Bonus AI Enrichment complete! 1 credit used. (${currentCredits - 1} remaining)`);
 } catch (err) {
 console.error("Error during Bonus AI Enrichment:", err);
 alert("Failed to run Bonus AI Enrichment. Please try again.");
 } finally {
 ...
 await recordDailyAction('note_edit');
 setPerson((prev: any) => prev ? ({ ...prev, ...editData }) : { ...displayPerson, ...editData });
 ...
 const roomsQuery = query(roomsRef, where('person_id', '==', trueId));
 ...
 <div className={`p-5 bg-white dark:bg-zinc-800 border ${accent.border} border-t border-t-white/5 rounded-3xl relative flex flex-col sm:flex-row items-start gap-4 shadow-sm dark:shadow-lg`}>
 ...
 {/* Hero Header */}
 <div className="bg-white dark:bg-zinc-800 border-b border-[var(--line)] pt-[var(--sat)]">
 ...
 memories: displayPerson?.memories || [],
 reflections: displayPerson?.reflections || [],
 photos: photoLog,
 gifts: displayPerson?.gifts || [],
 rooms: displayPerson?.rooms || []
 }}
 size="md"
 />
 </div>
 ...
 {/* Tab Switcher */}
 <div className="flex p-1 bg-zinc-100 dark:bg-zinc-800/80 border border-transparent dark:border-zinc-700 rounded-2xl">
 ...
 activeTab === tab 
 ? "bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white border border-transparent dark:border-zinc-700 border-t border-t-white/5 shadow-sm dark:shadow-lg" 
 ...
 className="flex items-center justify-center gap-2 p-4 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 border-t border-t-white/5 rounded-2xl font-bold shadow-sm dark:shadow-lg hover:scale-[1.02] transition-transform"
 >
 ...
 animate={{ opacity: 1, y: 0 }}
 exit={{ opacity: 0, y: 20 }}
 className={`p-6 ${accent.bgLight} dark:bg-zinc-800 border ${accent.border} border-t border-t-white/5 rounded-3xl space-y-6 shadow-xl dark:shadow-lg`}
 >
 <div className="flex justify-between items-center">
 ...
 <div className="bg-white dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700 border-t border-t-white/5 rounded-3xl p-2 space-y-1 shadow-sm dark:shadow-lg">
 ...
 <HelpTip 
 title="Close Friend Status" 
 content="Toggle Close Friend to pin this person to your story check-in rail and prioritize their countdowns on your main dashboard." 
 />
 </div>
 </div>
 <div className="bg-white dark:bg-zinc-800 rounded-[32px] border border-zinc-150 dark:border-zinc-700 border-t border-t-white/5 p-6 space-y-4 shadow-sm dark:shadow-lg">
 ...
 <div className="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-950 rounded-2xl border border-zinc-100 dark:border-zinc-700">
 ...
 <div className="bg-white dark:bg-zinc-800 rounded-3xl border border-zinc-100 dark:border-zinc-700 border-t border-t-white/5 p-6 space-y-4 shadow-sm dark:shadow-lg">
 ...
 className="w-full p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-700 text-sm min-h-[100px]"
 ...
 <div key={year} className="bg-white dark:bg-zinc-800 rounded-2xl border border-zinc-100 dark:border-zinc-700 border-t border-t-white/5 overflow-hidden shadow-sm dark:shadow-lg">
 <button 
 ...
 <p className="text-sm text-zinc-800 dark:text-zinc-200 bg-zinc-50 dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-700 p-4 rounded-xl leading-relaxed whitespace-pre-wrap">{existingReflection}</p>
 </div>
 ) : (
 <>
 ...
 required
 />
 <button type="submit" className="w-full py-3 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-xl font-bold text-sm hover:opacity-90 transition-opacity">
 Save Reflection
 </button>
 </form>
 </>
 ...
 className="p-6 bg-white dark:bg-zinc-800 rounded-3xl border border-zinc-100 dark:border-zinc-700 border-t border-t-white/5 space-y-4 overflow-hidden shadow-sm dark:shadow-lg"
 >
 <div className="space-y-1">
 ...
 className="w-full p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-700 text-sm"
 ...
 className="w-full p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-700 text-sm appearance-none"
 >
 ...
 <div key={gift.id} className="p-4 bg-white dark:bg-zinc-800 rounded-2xl border border-zinc-100 dark:border-zinc-700 border-t border-t-white/5 flex justify-between items-center shadow-sm dark:shadow-lg">
 <div className="flex items-center gap-3">
 ...
 <EmptyState 
 icon={Gift}
 title="No gift ideas added yet"
 description="Save gift ideas, wishlist items, or clothing sizes so you always know what to get them!"
 actionLabel="Add a Gift Idea"
 onAction={() => setShowGiftForm(true)}
 />
 )}
 </div>
 ...
 <div key={gift.id} className="p-4 bg-zinc-50 dark:bg-zinc-950 rounded-2xl border border-zinc-100 dark:border-zinc-700 flex justify-between items-center opacity-70">
 <div className="flex items-center gap-3">
 ...
 <div className="p-6 bg-white dark:bg-zinc-800 rounded-3xl border border-zinc-200 dark:border-zinc-700 border-t border-t-white/5 space-y-2 shadow-sm dark:shadow-lg">
 ...
 <div className={`p-6 bg-white dark:bg-zinc-800 rounded-3xl border ${accent.border} border-t border-t-white/5 space-y-4 shadow-sm dark:shadow-lg`}>
 ...
 <button
 onClick={handleBonusAIEnrichment}
 disabled={isEnrichingAI}
 className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer text-white shadow-sm ${accent.bgSolid} ${accent.bgSolidHover}`}
 title="Run instant AI profile synthesis using a Bonus AI credit"
 >
 <Sparkles size={13} className={isEnrichingAI ? "animate-spin" : ""} />
 <span>{isEnrichingAI ? "Synthesizing..." : `Enrich AI Notes (${user?.bonusEnrichmentCredits || 0})`}</span>
 </button>
 </div>
 ...
 <div className="p-6 bg-white dark:bg-zinc-800 rounded-3xl border border-zinc-200 dark:border-zinc-700 border-t border-t-white/5 space-y-6 shadow-sm dark:shadow-lg">
 <div className="flex justify-between items-center">
 <div>
 ...
 {isUploadingPhoto && (
 <div className={`p-4 bg-zinc-50 dark:bg-zinc-950 rounded-2xl flex flex-col items-center justify-center gap-2 border border-dashed border-zinc-200 dark:border-zinc-700 animate-pulse`}>
 ...
 <div key={item.id} className="group relative bg-zinc-50 dark:bg-zinc-950 border border-zinc-150 dark:border-zinc-700 rounded-2xl overflow-hidden flex flex-col p-3 gap-3 hover:shadow-md transition-all">
 <img 
 src={item.url} 
 alt="Shared memory" 
 ...
 <div className="text-center py-6 border border-dashed border-zinc-150 dark:border-zinc-700 rounded-2xl bg-zinc-50/50 dark:bg-zinc-950/50">
 ...
 animate={{ y: 0 }}
 exit={{ y: '100%' }}
 className="relative w-full max-w-lg bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 border-t border-t-white/5 rounded-t-[32px] sm:rounded-[32px] p-8 overflow-y-auto max-h-[90vh] shadow-2xl dark:shadow-lg"
 >
 ...
 {(editData.importance || 3) === 4 && "4 = Very Important"}
 {(editData.importance || 3) === 5 && "5 = Highest / VIP"}
 </span>
 </div>
 ...
 key={level}
 type="button"
 onClick={() => setEditData({ ...editData, importance: level })}
 className={`flex-1 py-3 rounded-xl font-bold transition-all cursor-pointer ${
 (editData.importance || 3) === level 
 ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900 shadow-lg' 
 : 'bg-zinc-50 dark:bg-zinc-950 text-zinc-400 border border-zinc-200 dark:border-zinc-700 hover:border-zinc-300'
 }`}
 >
 {level}
 </button>
