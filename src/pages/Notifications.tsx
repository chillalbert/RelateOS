...
export default function Notifications() {
  ...
  const handleDeleteRequest = async (requestId: string, senderName: string) => {
    if (!window.confirm(`Are you sure you want to delete the friend request from ${senderName}? This action is permanent.`)) return;
    try {
                        ...
                        <div className="flex gap-2 items-center">
                          {isAccepted || isDeclined ? (
                            ...
                            <>
                              ...
                              <button
                                onClick={() => handleDeleteRequest(invite.id, invite.sender_name)}
                                className="p-2 text-zinc-400 hover:text-red-500 hover:bg-red-500/10 dark:hover:bg-red-500/20 rounded-xl transition-all cursor-pointer"
